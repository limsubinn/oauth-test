package com.example.oauthtest._common.filter;

import com.example.oauthtest._common.utils.JwtCode;
...
public class JwtAuthorizationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        // access token 이 유효
        if (jwtTokenProvider.validateToken(accessToken) == JwtCode.ACCESS) {
            // memberId -> member 찾기
        ...
        // access token 이 만료
        else if (jwtTokenProvider.validateToken(accessToken) == JwtCode.EXPIRED) {
